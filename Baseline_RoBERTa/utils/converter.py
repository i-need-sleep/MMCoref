...
def dial_KB_to_string(split = "train", dial_mem = 1, KB_hist = False, KB_cur = True, KB_cand = True, out_path = 'test'):
    ...
    for dial_idx, dial in enumerate(tqdm.tqdm(dials)):
        ...
        for round_idx, cur_round in enumerate(dial_hist):
            ...
            for r in range(max(0, round_idx - dial_mem), round_idx):
                ...
                if KB_cand:
                    round_out += KB_retrive_cand(round_scene_id, ref_KB)
                
                if not KB_cur:
                    for idx, obj in enumerate(cur_round["transcript_annotated"]["act_attributes"]["objects"]):
                            cur_round["transcript_annotated"]["act_attributes"]["objects"][idx] = str(obj)
                    round_out += f'=> Predict : {" ".join(cur_round["transcript_annotated"]["act_attributes"]["objects"])}\n'
                else:
                    KB_out = KB_retrive_idx(cur_round["transcript_annotated"]["act_attributes"]["objects"], round_scene_id, ref_KB)
                    if KB_out == 'break':
                        break
                    round_out += f'=> Predict : {KB_out}\n'
                out += round_out.replace('  ',' ')
                if len(round_out) > max_len:
                    max_len = len(round_out)
                    print(round_out, max_len)
                continue
            break
    ...
    with open(f'{OUT_ROOT}/{out_path}.txt', 'w', encoding='utf-8') as out_file:
        out_file.write(out)
...
def KB_retrive_cand(scene_id, KB):
    scene_root = f"{DATA_ROOT}/simmc2_scene_jsons_dstc10_public/public"
    scene_path = f'{scene_root}/{scene_id}_scene.json'
