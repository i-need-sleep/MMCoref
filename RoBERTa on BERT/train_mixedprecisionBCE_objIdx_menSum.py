...
from scripts.dataset_objIndex_men import make_loader_with_index
...
def train():
    ...
    # Setup Tensorboard
    writer = SummaryWriter(comment = f'RobertaOnBERT_ObjidxMenSum batch_size={BATCH_SIZE}, Adam_lr={LR}, BCEweigh={ALPHA}')
    ...
    class Roberta_on_KB_emb(torch.nn.Module):
        def __init__(self, device):
            ...
            self.obj_id_emb = torch.nn.Embedding(200, 768, padding_idx=0)
            self.obj_men_emb = torch.nn.Embedding(200, 768, padding_idx=0)
            # self.emb_bag  = torch.nn.Linear(768*4, 768)
        ...
        def forward(self, tokens, attn_mask, KB_embs, obj_ids, obj_men):
            KB_embs = self.lin_KB(KB_embs) # (batch, 512, 768)
            obj_id_embs = self.obj_id_emb(obj_ids)
            obj_men_embs = self.obj_men_emb(obj_men)
            emb = self.roberta.embeddings(tokens)
            emb = emb + KB_embs + obj_id_embs + obj_men_embs
            # emb = self.emb_bag(torch.cat((emb, KB_embs, obj_id_embs, obj_men_embs), dim=2))
    ...
    def eval(model):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(dev_loader):
                dial_tokens, roberta_attn_mask, obj_embs, obj_men, obj_ids, output_mask, reference = (i.to(device) for i in batch)

                pred = model(dial_tokens, roberta_attn_mask, obj_embs, obj_men, obj_ids)
    ...
    for epoch in range(N_EPOCH):
        for batch_idx, batch in enumerate(train_loader):
            ...
            dial_tokens, roberta_attn_mask, obj_embs, obj_ids, obj_men, output_mask, reference = (i.to(device) for i in batch)
            ...
            if batch_idx % 500 == 0:
                ...
                torch.save({
                    ...
                    'dev_loss': loss,
                    }, f'./checkpoint/RobertaOnBERT_mixed_ObjIdxMenSum_batchsize{BATCH_SIZE}_lr{LR}_BCEweigh{ALPHA}_{epoch}_{batch_idx}_{loss}_{f1}.bin')
    print('DONE !!!')
