...
class Modified_Uniter(torch.nn.Module):
    def __init__(self):
        ...
        self.emb_bag = torch.nn.Linear(2048+3*128, 2048)
    ...
    def forward(self, input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask):
        ...
        vis_feats = torch.cat((vis_feats, obj_embs, obj_id_embs, pos_x_emb, pos_y_emb, pos_z_emb), axis=2)
...
if __name__ == '__main__':
    model = Modified_Uniter()
    from dataset import make_loader
    loader = make_loader('dev', 16)
    for idx, data in enumerate(loader):
        if idx == 1:
            input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference = data
            out = model(input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask)
            print(out.shape)
            out=out.reshape(1,-1)
            print(output_mask.shape)
            print(out[output_mask==1].shape)
