...
class UNITER_on_CLIP_BERT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dial, objects, reference, obj_ids, obj_pos, obj_bbox, scenes, KB_ids = line['dial'], line['objects'], line['reference_mask'], line['candidate_ids'], line['candidate_pos'], line['candidate_bbox'], line['scenes'], line['KB_ids']
        ...
        return input_ids , txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference, dial_idx, round_idx

def mr_collate(data):
    
    input_ids , txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference, dial_idx, round_idx = data[0]
    ...
    for idx, line in enumerate(data):
        if idx == 0:
            continue
        input_ids_l , txt_seg_ids_l, vis_feats_l, KB_ids_l, obj_ids_l, pos_x_l, pos_y_l, pos_z_l, bboxes_l, vis_seg_l, extended_attention_mask_l, output_mask_l, reference_l, dial_idx_l, round_idx_l = line 
...
def make_KBid_loader(split, batch_size):
