...
class RoBERTa_on_BERT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dial, objects, reference, obj_ids, obj_men = line['dial'], line['objects'], line['reference_mask'], line['candidate_ids'], line['candidate_men']
        ...
        obj_ids = torch.tensor([obj_ids]) + 1 # add 1 to the index to distinguish index 0 and padding 0
        obj_ids = torch.nn.ZeroPad2d((dial_token_len, len(dial_tokens) - dial_token_len - obj_ids.shape[1],0,0))(obj_ids)
       
        obj_men = torch.tensor([obj_men]) + 1 
        obj_men = torch.nn.ZeroPad2d((dial_token_len, len(dial_tokens) - dial_token_len - obj_men.shape[1],0,0))(obj_men)

        # for i in (dial_tokens, roberta_attn_mask, obj_embs, output_mask, reference):
        #     print(torch.tensor(i).shape)    

        return dial_tokens, roberta_attn_mask, obj_embs, obj_ids, obj_men, output_mask, reference


def make_loader_with_index(split, batch_size):
    dataset = RoBERTa_on_BERT_Dataset(split)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=mr_collate)
    return loader

def mr_collate(data):
    dial_tokens, roberta_attn_mask, obj_embs, obj_ids, obj_men, output_mask, reference = (torch.tensor(i) for i in data[0])
    ...
    for idx, line in enumerate(data):
        if idx == 0:
            continue
        dial_tokens_l, roberta_attn_mask_l, obj_embs_l, obj_ids_l, obj_men_l, output_mask_l, reference_l = (torch.tensor(i) for i in line)
...
if __name__ == "__main__":
    testData = RoBERTa_on_BERT_Dataset('dev')
    print(testData[0])
    loader = make_loader_with_index('dev', 16)
