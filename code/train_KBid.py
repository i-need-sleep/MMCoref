...
from Transformers_VQA.dataset_KBid import make_KBid_loader
from Transformers_VQA.modified_uniter_KBid import Modified_Uniter_KBid

def make_KBid_emb_init(emb_mat):
    KBid_emb_init = torch.load('./processed/KB_emb.pt', map_location=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
...
def train():
    ...
    GAMMA = 2
    ALPHA = 5
    print(f'KBid batch_size={BATCH_SIZE}, Adam_lr={LR}, FocalAlpha={ALPHA}, GAMMA={GAMMA}')
    ...
    # Make loaders
    train_loader = make_KBid_loader('train', BATCH_SIZE)
    dev_loader = make_KBid_loader('dev', BATCH_SIZE_DEV)

    # Setup Tensorboard
    writer = SummaryWriter(comment = f'KBid batch_size={BATCH_SIZE}, Adam_lr={LR}, FocalAlpha={ALPHA}, GAMMA={GAMMA}')
    ...
    def eval(model):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(dev_loader):
                input_ids, txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference, _, _ = batch
                input_ids, txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference = (i.to(device) for i in (input_ids, txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference))
    ...
    emb_mat = state_dict['KBid_emb.weight']
    state_dict['KBid_emb.weight'] = make_KBid_emb_init(emb_mat)
    ...
    for epoch in range(N_EPOCH):
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx % 2000 == 0:
                ...
                try:
                    os.makedirs(f'./checkpoint/KBid_batchsize{BATCH_SIZE}_lr{LR}_FocalALPHA{ALPHA}_GAMMA{GAMMA}')
                except:
                    pass
                ...
                torch.save({
                    ...
                    'dev_loss': loss,
                    }, f'./checkpoint/KBid_batchsize{BATCH_SIZE}_lr{LR}_FocalALPHA{ALPHA}_GAMMA{GAMMA}/{epoch}_{batch_idx}_{loss}_{f1}.bin')
    print('DONE !!!')
