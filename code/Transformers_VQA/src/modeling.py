...
class BertAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        # Attention bias for the four relaionships
        self.rel_emb_left = nn.Embedding(13, 1, padding_idx=0)
        self.rel_emb_right = nn.Embedding(13, 1, padding_idx=0)
        self.rel_emb_up = nn.Embedding(13, 1, padding_idx=0)
        self.rel_emb_down = nn.Embedding(13, 1, padding_idx=0)
    ...
    def forward(self, hidden_states, context, attention_mask=None, rel_masks=None):
        ...
        # Attention bias
        if rel_masks != None:
            rel_mask_left, rel_mask_right, rel_mask_up, rel_mask_down = rel_masks[0], rel_masks[1], rel_masks[2], rel_masks[3]
            rel_bias_left = self.rel_emb_left(rel_mask_left.long()).squeeze(-1)
            rel_bias_right = self.rel_emb_right(rel_mask_right.long()).squeeze(-1)
            rel_bias_up = self.rel_emb_up(rel_mask_up.long()).squeeze(-1)
            rel_bias_down = self.rel_emb_down(rel_mask_down.long()).squeeze(-1)
            attention_scores = attention_scores + rel_bias_left + rel_bias_right + rel_bias_up + rel_bias_down
...
class BertGraphAttention(nn.Module):
    def __init__(self, config, ctx_dim=None):
        ...
        # Attention bias for the four relaionships
        self.rel_key = nn.Embedding(1, self.all_head_size)
        self.rel_val = nn.Embedding(1, self.all_head_size)
    ...
    def forward(self, hidden_states, context, attention_mask=None, rel_mask=None):
        attention_mask = rel_mask
        mixed_query_layer = self.query(hidden_states)
        mixed_key_layer = self.key(context) + self.rel_key(self.zeros_like(context))
        mixed_value_layer = self.value(context) + self.rel_val(self.zeros_like(context))
...
class BertSelfattLayer(nn.Module):
    def __init__(self, config):
        ...
        self.graph_attn_left = BertGraphAttention(config)
        self.graph_attn_right = BertGraphAttention(config)
        self.graph_attn_up = BertGraphAttention(config)
        self.graph_attn_down = BertGraphAttention(config)
        self.output = BertAttOutput(config)

    def forward(self, input_tensor, attention_mask, rel_masks=None, graph_attn=False):
        # Self attention attends to itself, thus keys and querys are the same (input_tensor).
        if not graph_attn:
            self_output = self.self(input_tensor, input_tensor, attention_mask, rel_masks)
        else:
            self_output = self.self(input_tensor, input_tensor, attention_mask, None)
            self_graph_output_left = self.graph_attn_left(input_tensor, input_tensor, attention_mask, rel_masks[0])
            self_graph_output_right = self.graph_attn_right(input_tensor, input_tensor, attention_mask, rel_masks[1])
            self_graph_output_up = self.graph_attn_up(input_tensor, input_tensor, attention_mask, rel_masks[2])
            self_graph_output_down = self.graph_attn_down(input_tensor, input_tensor, attention_mask, rel_masks[3])
            self_output = self_output + self_graph_output_left + self_graph_output_right + self_graph_output_up + self_graph_output_down
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, rel_masks=None, graph_attn=False):
        attention_output = self.attention(hidden_states, attention_mask, rel_masks, graph_attn=False)
...
class BertPreTrainedModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir='./cache',
...
class BertEncoder(nn.Module):
    ...
    def forward(self,hidden_states,attention_mask=None, rel_masks=None, graph_attn=False):
        all_attentions = ()
        for i, layer_module in enumerate(self.layer):
            layer_outputs = layer_module(hidden_states,attention_mask, rel_masks, graph_attn)
...
class UniterModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, txt_type_ids=None,attention_mask=None,
                visual_attention_mask=None,img_feats=None,img_pos_feat=None,img_type_ids=None, graph_attn=False): 
        attention_mask = torch.cat((attention_mask,visual_attention_mask),dim=1)
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        # print(extended_attention_mask.shape) (batch, 1, 1, seq_len)
        # print(input_ids.shape) (batch, txt_len)
        # print(txt_type_ids.shape)
        # print(img_feats.shape) (batch, img_len, 2048)
        # print(img_pos_feat.shape) (batch, img_len, 7)
        # print(img_type_ids.shape) (batch, img_len)
        ...
        lang_v_feats = self.encoder(hidden_states = embeddings, attention_mask = extended_attention_mask, graph_attn=graph_attn)
        pooled_output = self.pooler(lang_v_feats)
        return pooled_output
