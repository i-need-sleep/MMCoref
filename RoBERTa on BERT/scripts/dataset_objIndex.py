...
class RoBERTa_on_BERT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dial, objects, reference, obj_ids = line['dial'], line['objects'], line['reference_mask'], line['candidate_ids']
        ...
        return dial_tokens, roberta_attn_mask, obj_embs, obj_ids, output_mask, reference
...
def mr_collate(data):
    dial_tokens, roberta_attn_mask, obj_embs, obj_ids, output_mask, reference = (torch.tensor(i) for i in data[0])
    ...
    for idx, line in enumerate(data):
        if idx == 0:
            continue
        dial_tokens_l, roberta_attn_mask_l, obj_embs_l, obj_ids_l, output_mask_l, reference_l = (torch.tensor(i) for i in line)
