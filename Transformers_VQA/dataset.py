...
class UNITER_on_CLIP_BERT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        dial, objects, reference, obj_ids, obj_pos, obj_bbox, scenes = line['dial'], line['objects'], line['reference_mask'], line['candidate_ids'], line['candidate_pos'], line['candidate_bbox'], line['scenes']
        ...
        return input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference

def mr_collate(data):
    
    input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference = data[0]
    ...
    for idx, line in enumerate(data):
        if idx == 0:
            continue
        input_ids_l , txt_seg_ids_l, vis_feats_l, obj_embs_l, obj_ids_l, pos_x_l, pos_y_l, pos_z_l, bboxes_l, vis_seg_l, extended_attention_mask_l, output_mask_l, reference_l = line 
        ...
        obj_embs = torch.cat((obj_embs, obj_embs_l), axis=0)
...
if __name__ == "__main__":
    testData = UNITER_on_CLIP_BERT_Dataset('dev')
    # print(testData[5])
    loader = make_loader('train', 16)
    max_pos = 0
    min_pos = 0
    for i, s in enumerate(loader):
        ...
        input_ids, txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference = s
        for pos in [pos_x, pos_y, pos_z]:
            if torch.max(pos) > max_pos:
                max_pos = torch.max(pos)
            if torch.min(pos) < min_pos:
