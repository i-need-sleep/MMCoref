...
class UNITER_on_CLIP_BERT_Dataset(Dataset):
    def __init__(self, split, max_n_obj=200, rcnn=False):
    ...
    def __getitem__(self, index):
        ...
        # Make the vis feat tensor (#obj + #scene, 512+1024)
        ...
        if self.rcnn:
            ...
            vis_feats = torch.cat((vis_feats, vis_feats_rcnn.to(self.device)), dim=1)
        ...
        return input_ids , txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference, dial_idx, round_idx, obj_embs, scene_segs, rel_mask_left, rel_mask_right, rel_mask_up, rel_mask_down, obj_embs_SBERT

def mr_collate(data):
    
    input_ids , txt_seg_ids, vis_feats, KB_ids, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, output_mask, reference, dial_idx, round_idx, obj_embs, scene_segs, rel_mask_left, rel_mask_right, rel_mask_up, rel_mask_down, obj_embs_SBERT = data[0]
    ...
    for idx, line in enumerate(data):
        if idx == 0:
            continue
        input_ids_l , txt_seg_ids_l, vis_feats_l, KB_ids_l, obj_ids_l, pos_x_l, pos_y_l, pos_z_l, bboxes_l, vis_seg_l, extended_attention_mask_l, output_mask_l, reference_l, dial_idx_l, round_idx_l, obj_embs_l, scene_segs_l, rel_mask_left_l, rel_mask_right_l, rel_mask_up_l, rel_mask_down_l, obj_embs_SBERT_l = line 
...
def make_final_loader(split, batch_size, rcnn=False):
    dataset = UNITER_on_CLIP_BERT_Dataset(split, rcnn=rcnn)
