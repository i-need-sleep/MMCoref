...
class Modified_Uniter_sceneseg(torch.nn.Module):
    def __init__(self):
        super(Modified_Uniter_sceneseg, self).__init__()
        ...
        # Convert input (Ojb_idx: 512, KB: 1024, visual: 512, pos: 3*128, scene_seg: 128)
        ...
        self.emb_bag = torch.nn.Linear(2048+512, 2048)

        # Convert output
        self.clsHead = torch.nn.Linear(768, 1)
            
    def forward(self, input_ids , txt_seg_ids, vis_feats, obj_embs, obj_ids, pos_x, pos_y, pos_z, bboxes, vis_seg, extended_attention_mask, scene_seg):
