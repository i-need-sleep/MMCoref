...
from Transformers_VQA.modified_uniter_attnbias import Modified_Uniter_attnbias
...
def train():
    ...
    GAMMA = 2
    ALPHA = 5
    print(f'UNITERonCLIPBERT_attnbias batch_size={BATCH_SIZE}, Adam_lr={LR}, FocalAlpha={ALPHA}, GAMMA={GAMMA}')
    ...
    # Make loaders
    train_loader = make_final_loader('train', BATCH_SIZE)
    dev_loader = make_final_loader('dev', BATCH_SIZE_DEV)

    # Setup Tensorboard
    writer = SummaryWriter(comment = f'UNITERonCLIPBERT_attnbias batch_size={BATCH_SIZE}, Adam_lr={LR}, FocalAlpha={ALPHA}, GAMMA={GAMMA}')
    ...
    def eval(model):
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(dev_loader):
                ...
                rel_mask_left = batch['rel_mask_left'].to(device)
                rel_mask_right = batch['rel_mask_right'].to(device)
                rel_mask_up = batch['rel_mask_up'].to(device)
                rel_mask_down = batch['rel_mask_down'].to(device)

                mask_stepper = torch.ones(1, 12, 512, 512).to(device)
                for i in range(12):
                    mask_stepper[0, i, :, :] *= i+1
                rel_masks = [rel_mask_left, rel_mask_right, rel_mask_up, rel_mask_down]
                for i, mask in enumerate(rel_masks):
                    rel_masks[i] = mask.unsqueeze(1)*mask_stepper
    ...
    # Training setup
    model = Modified_Uniter_attnbias().to(device)
    ...
    for epoch in range(N_EPOCH):
        for batch_idx, batch in enumerate(train_loader):
            ...
            if batch_idx % 2000 == 0:
                ...
                try:
                    os.makedirs(f'./checkpoint/UNITERonCLIPBERT_attnbias_batchsize{BATCH_SIZE}_lr{LR}_FocalALPHA{ALPHA}_GAMMA{GAMMA}')
                except:
                    pass
                ...
                torch.save({
                    ...
                    'dev_loss': loss,
                    }, f'./checkpoint/UNITERonCLIPBERT_attnbias_batchsize{BATCH_SIZE}_lr{LR}_FocalALPHA{ALPHA}_GAMMA{GAMMA}/{epoch}_{batch_idx}_{loss}_{f1}.bin')
    print('DONE !!!')
